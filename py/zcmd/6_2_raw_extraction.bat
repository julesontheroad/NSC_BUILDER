...
ECHO ***********************************************
echo.
%pycommand% "%sq_lc%" -lib_call cmd.cmd_tools userinput -xarg "%list_folder%\6_2list.txt" ext="nsp xci nsz xcz nsx" userfile="%uinput%"
set /p eval=<"%uinput%"
set eval=%eval:"=%
setlocal enabledelayedexpansion
echo+ >"%uinput%"
...
set bs=%bs:"=%
%pycommand% "%sq_lc%" -lib_call listmanager remove_from_botton -xarg  "%list_folder%\6_2list.txt" "%bs%"

:showlist
cls
call "%nscb_logos%" "program_logo"
%pycommand% "%sq_lc%" -lib_call listmanager printcurrent -xarg  "%list_folder%\6_2list.txt" "all" "counter=True"
goto checkagain
...
:s_exit_choice
if exist "%prog_dir%lists\6_2list.txt" ( call "%nscb_tools%" "delete_if_empty" "6_2list.txt" )
