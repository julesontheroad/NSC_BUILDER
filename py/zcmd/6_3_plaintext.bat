...
ECHO ***********************************************
echo.
%pycommand% "%squirrel%" -t nsp xci -tfile "%list_folder%\6_3list.txt" -uin "%uinput%" -ff "uinput"
set /p eval=<"%uinput%"
set eval=%eval:"=%
setlocal enabledelayedexpansion
echo+ >"%uinput%"
...
if /i "%eval%"=="1" ( %pycommand% "%squirrel%" -lib_call listmanager selector2list -xarg "%list_folder%\6_3list.txt" mode=folder ext="nsp xci" ) 2>&1>NUL
if /i "%eval%"=="2" ( %pycommand% "%squirrel%" -lib_call listmanager selector2list -xarg "%list_folder%\6_3list.txt" mode=file ext="nsp xci" ) 2>&1>NUL
if /i "%eval%"=="3" ( %pycommand% "%squirrel%" -lib_call picker_walker select_from_local_libraries -xarg "%list_folder%\6_3list.txt" "extlist=nsp xci" )
if /i "%eval%"=="4" ( %pycommand% "%squirrel%" -lib_call picker_walker get_files_from_walk -xarg "%list_folder%\6_3list.txt" "extlist=nsp xci" )
goto checkagain
echo.
:checkagain
...
if /i "%eval%"=="2" ( %pycommand% "%squirrel%" -lib_call listmanager selector2list -xarg  "%list_folder%\6_3list.txt" mode=folder ext="nsp xci" ) 2>&1>NUL
if /i "%eval%"=="3" ( %pycommand% "%squirrel%" -lib_call listmanager selector2list -xarg  "%list_folder%\6_3list.txt" mode=file ext="nsp xci" )  2>&1>NUL
if /i "%eval%"=="4" ( %pycommand% "%squirrel%" -lib_call picker_walker select_from_local_libraries -xarg "%list_folder%\6_3list.txt" "extlist=nsp xci" )
if /i "%eval%"=="5" ( %pycommand% "%squirrel%" -lib_call picker_walker get_files_from_walk -xarg "%list_folder%\6_3list.txt" "extlist=nsp xci" )
...
(for /f "tokens=1,*delims=:" %%a in (' findstr /n "^" ^<lists/6_3list.txt'
) do Echo=%skip%|findstr ",%%a," 2>&1>NUL ||Echo=%%b
)>lists/6_3list.txt.new
endlocal
move /y "lists/6_3list.txt.new" "lists/6_3list.txt" >nul
endlocal
...
:s_exit_choice
call "%nscb_tools%" "delete_if_empty" "6_3list.txt"
